BUSINESS_CONTEXT = """
## About Glam by Dang

**Business Name:** Glam by Dang NYC
**Owner:** Kha Fitzpatrick (also known as Kha Dang)
**Established:** 2022
**Location:** 37 West 26th Street, Suite 808 (8th Floor), New York, NY 10010
**Neighborhood:** Flatiron District, Manhattan

**Contact:**
- Email: glambydang@gmail.com
- Instagram: @glambydangnyc
- Website: glambydang.com
- Booking: https://www.fresha.com/a/glam-by-dang-new-york-37-west-26th-street-qthgd9hw/booking?menu=true&pId=599800&dppub=true&utm_content=link_in_bio&utm_medium=social&utm_source=ig&employeeId=1757692&cartId=6cbcfff7-65bc-4ba4-ad4d-e67846bda781

**Hours:** Tuesday–Friday 11am–8pm, Saturday 10am–4pm. Closed Sunday and Monday. By appointment only — clients book through Kha's booking link or by DMing directly.

---

## Finding the Studio

The studio is at 37 West 26th Street, Suite 808, 8th Floor, New York, NY 10010 (Flatiron District).

**Important:** GPS sometimes directs clients to the old location at 65 W. 36th St — that is incorrect. The current address is 37 W. 26th St, and the studio is on the 8th floor, Suite 808.

If a client says they're at 36th St or asks about the building, they've been sent to the wrong location by GPS. Direct them to 37 W. 26th St, 8th floor, Suite 808 — ring #808 on the directory to the left of the elevator.

---

## About Kha

Kha was born and raised in Vietnam and moved to the United States at age 19. She discovered her passion for the beauty industry while in college, pursued multiple certifications, and founded Glam by Dang in 2022. Her philosophy is to create beauty and wellness experiences while collaborating with like-minded small businesses in her community. She is known for her warm energy, precision, and making clients feel genuinely beautiful — not just look beautiful.

---

## Services & Pricing

### Fine Line Tattoo
- Flash tattoo: starts at $100 — pre-designed flash options can be found in the Flash highlight on Kha's profile. Booking more than one flash tattoo brings the per-tattoo price down.
- Flash tattoo placement: arms and legs only. A flash design placed anywhere else (ribs, hands, feet, neck, back, etc.) is priced and booked as a custom tattoo instead, starting at $150 — even though it's a pre-made design.
- Custom tattoo: starts at $150 — includes 1 quote/word, cursive letter, or number. Final price always depends on size, complexity, and placement. Clients share their reference photo or design idea here and Kha follows up with an exact quote.
- 2-week fill: from $100 — for existing tattoo clients, 9–15 days after original appointment.
- Ink colors: Kha works in red, brown, and black ink.

### Brow Shaping
- Brow shape: from $30 (15–30 min).

### Permanent Makeup (starting prices ~$750 per service)
- Nanoblading
- Hairstroke brows
- Microblading
- Powder brow / ombre brow
- Lip blush: $750
- Eyeliner tattoo: $750
- Brow tattoo: $750
- Faux freckles: $150

### Keratin Services & Tinting
- Keratin lash lift: $175 — lifts and enhances natural lashes without extensions; lash tinting is included.
- Brow lamination: $150
- Tinting (lash or brow): $30
- Keratin lash + brow combo: $300 (~8% savings)

### Lash Extensions
- Classic full set: $200
- Hybrid full set: $250
- Volume full set: $300
- Classic fill: $100 — fills should be booked every 2–3 weeks (9–15 days ideal window).
- Hybrid fill: $105
- Volume fill: $115
- Note: anything over 4 weeks since last appointment requires a new full set, not a fill.

**Overall price range:** $20 – $3,500 depending on service complexity.

### Tattoo Parties (Events)
Kha is available for weddings, birthdays, bachelorettes, corporate events, and other celebrations. Guests receive free flash tattoos (1 per guest), with simple placements (arms, hands, legs, etc.).

**Packages:**
- **2 Hour Flash Tattoo** — $1,000 (for events with 20–30 guests). Pre-designed flash sheets, no custom branding.
- **4 Hour Flash Tattoo** — $2,000 (for events with up to 50 guests). Custom branded design sheet with ~10 tiny tattoo designs.
- **6 Hour Flash Tattoo** — $3,500 (for events with up to 100 guests). Custom branded design sheet with ~20 tiny tattoo designs.

For event inquiries, clients share details here and Kha follows up directly.

### Other Offerings
- **Memberships:** Service bundles available — clients can ask Kha for details.
- **Gift cards:** Available for purchase online.
- **Products:** Available through her online store.

---

## Policies

**Booking:**
- Appointments are per person. If multiple people want services (e.g. a group getting matching flash tattoos), each person books their own separate appointment — this does not apply to Kha's tattoo party packages for events, which are already priced per group.

**Deposits:**
- A deposit is required to book an appointment
- Deposits are non-refundable
- Cancellations made at least 7 days in advance receive a credit toward a future booking (not a cash refund)
- Cancellations made less than 7 days in advance are not eligible for a refund or credit

**No-Shows & Late Cancellations:**
- Cancelling within 24 hours of the appointment, or not showing up, results in 100% of the appointment value being charged (not just the deposit)

**Late Arrivals:**
- If you arrive more than 15 minutes late, the appointment will be cancelled

**Reminders:**
- Appointment reminders are sent 3 days in advance via text and email

**Other:**
- Please do not come to your appointment if you are feeling ill
- Pricing and policies are subject to change at any time

---

## Pre & Post Care

### Permanent Makeup — Brows (Nanoblading, Microblading, Powder Brow, etc.)

**Before your appointment:**
- No tweezing, waxing, or electrolysis 1 week before
- No tanning or sun exposure 2 weeks before; no sunburned skin
- No botox 3 weeks before
- No facials 2 weeks before
- Avoid fish oil and vitamin E supplements 1 week before
- No working out, alcohol (24–48 hrs), coffee, aspirin, or ibuprofen the day of

**After your appointment:**
- Gently blot with clean tissue every 5 minutes on day of treatment
- Wash twice daily with antibacterial soap (gentle fingertip motions, 10 seconds) for days 1–7
- Apply a rice-grain size of aftercare ointment with a cotton swab
- Avoid acids, exfoliants, glycolic or lactic acid products for 1 week
- Use a fresh pillowcase nightly
- No hot, sweaty exercise for 1 week
- No facials, botox, or chemical treatments for 4 weeks
- No direct sun exposure or tanning for 4 weeks
- No long hot showers for 10 days; no swimming or hot tubs for 10 days
- Don't sleep face-down initially

### Lip Blush

**Before your appointment:**
- Schedule fillers at least 2 weeks prior
- Stop retinol at least 2 weeks before (ideally 4 weeks)
- Hydrate lips well in the week before; exfoliate nightly with lip scrub + hydrating treatment
- No alcohol, caffeine, aspirin, ibuprofen, fish oil, niacin, or vitamin E 48 hours before
- Shower before your appointment (avoid steam during healing)
- If prone to cold sores, start antiviral medication (Valtrex/Zovirax) as directed by your doctor
- Avoid scheduling during your menstrual cycle if possible (increased sensitivity)

**After your appointment:**
- Day 1 only: gently clean with a water-dampened cotton round
- Take ibuprofen or use an ice pack as needed for swelling
- Avoid hot food/drinks for 72 hours; drink through straws; avoid spicy or salty foods
- Skip tooth brushing for 24 hours (use mouthwash instead)
- Apply provided ointment regularly for 7 days; apply a thick layer before bed
- No picking or peeling — let skin shed naturally (Days 3–10)
- No lip cosmetics during healing
- Avoid physical lip contact for 7 days
- Full healing takes 6–8 weeks; color will appear to fade before settling — this is normal
- A touch-up appointment is required (mandatory)
- No lip injections 4 weeks before or after treatment

---

## Reputation & Reviews

- **Google:** 5.0 stars, 76 reviews
- **Online booking platform:** 5.0 stars, 139 reviews
- **Birdeye:** 5.0 stars, 76 reviews
- Also listed on The Knot and WeddingWire for bridal/wedding services

**Notable client quote:** "On the day of my wedding she was punctual and extremely professional. She has a wonderful energy about her — not only does she make you look beautiful but she makes you feel beautiful."

---

## What Makes Glam by Dang Special

- Perfect 5-star rating across every platform
- Kha personally performs all services — clients always work directly with her
- Specializes in the intersection of fine line tattoo artistry and permanent beauty enhancement
- Warm, intimate studio experience — not a chain or high-volume salon
- Popular for bridal and special occasion glam
- Serves clients seeking long-lasting, low-maintenance beauty solutions

---

## Review Links (for follow-up prompts)
- Google: Search "Glam by Dang NYC" on Google Maps to leave a review
"""
